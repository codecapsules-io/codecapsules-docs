...
Next, we have to install the dependencies we’ll need to build our application. Open the `composer.json` file in the root folder and add `ourcodeworld/name-that-color": "dev-master ` as an entry to the require dictionary. Afterwhich run the command `composer update` from the terminal to install the package.
