...
After adding your custom domain on Code Capsules, you need to create an _A record_ for the domain you've purchased. You'll use the IP that was saved [previously](custom_domains.md#adding-a-domain) when creating the A record for your domain. You can add an A record to your purchased domain by visiting your domain registrar's website.
...
* `provided-IP-Address` is the IP address found [previously](custom_domains.md#how-to-add-a-custom-domain) under "A Record Route"
...
Before adding a _subdomain_ to your application hosted on Code Capsules, make sure you have already [created an A record](custom_domains.md#how-to-add-a-custom-domain) for your purchased domain that routes to an application on Code Capsules.
...
To add a subdomain for your application, follow the same process detailed [previously](custom_domains.md#adding-a-domain), with some changes:
